...
def extract_some_patches(features, patch_dim, stride=(1, 1), patches_per_image=None, whiten=False, padding=0):
    ...
    if patches_per_image is not None:
        ...
        for i in range(batch_size):
            if patches.shape[1] > 1 or patches.shape[2] > 1:
                ...
                good_idxs = torch.nonzero(torch.norm(patches[i, :], 2, 1))
            if len(good_idxs) > 0:
                ...
                elif sampled_patches.dim() == 0:
                    sampled_patches = sampled_patches.unsqueeze(0).unsqueeze(0)

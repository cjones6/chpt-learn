...
def train(train_loader, valid_loader, test_loader, model, nclasses, maxiter, w_init=None, loss_name='mnl', lambdas=None,
    ...
    if isinstance(x_train, list):
        ...
        y_train = torch.cat(y_train).view(-1).type(torch.LongTensor)
        x_valid = torch.cat(x_valid)
        y_valid = torch.cat(y_valid).view(-1).type(torch.LongTensor)

    x_train, x_valid = x_train.to(defaults.device), x_valid.to(defaults.device)
    y_train, y_valid = y_train.to(defaults.device), y_valid.to(defaults.device)

    if x_test is not None:
        x_test = x_test.to(defaults.device)
        y_test = y_test.to(defaults.device)
